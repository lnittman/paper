...
describe("skills", () => {
	describe("loadSkillsFromDir", () => {
		...
		it("should skip uppercase documentation files (AGENTS.md, README.md, CLAUDE.md) at root level", () => {
			const { skills, diagnostics } = loadSkillsFromDir({
				dir: join(fixturesDir, "doc-files"),
				source: "test",
			});

			// Should only load the valid child skill, not the uppercase doc files
			expect(skills).toHaveLength(1);
			expect(skills[0].name).toBe("valid-child");
			// Should not produce any diagnostics about missing descriptions for doc files
			expect(
				diagnostics.filter((d: ResourceDiagnostic) => d.message.includes("description is required")),
			).toHaveLength(0);
		});
